...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose2d(input_size, gen_filter * 8, kernel_size=4, stride=1, padding=0, bias=False),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(dis_filter * 8, output_size, kernel_size=4, stride=1, padding=0, bias=False),
            nn.Sigmoid()
        )
