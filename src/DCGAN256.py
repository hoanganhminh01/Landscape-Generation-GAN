...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose2d(input_size, gen_filter * 32, kernel_size=4, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(gen_filter * 32),
            nn.ReLU(),
            nn.ConvTranspose2d(input_size * 32, gen_filter * 16, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(gen_filter * 16),
            nn.ReLU(),
            nn.ConvTranspose2d(gen_filter * 16, gen_filter * 8, kernel_size=4, stride=2, padding=1, bias=False),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(dis_filter * 16, dis_filter * 32, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(dis_filter * 32),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(dis_filter * 32, output_size, kernel_size=4, stride=1, padding=0, bias=False),
            nn.Sigmoid()
        )
